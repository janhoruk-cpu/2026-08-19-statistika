      ...
      <div className="max-w-2xl mx-auto space-y-10 pb-24">
        ...
        <header className="space-y-2 pb-6 border-b border-slate-200 dark:border-slate-800">
          <p className="text-base text-slate-500 dark:text-slate-400">
            {unit.unitNumber?.startsWith('Uvod') ? unit.unitNumber : `Učna enota ${unit.unitNumber || unit.id.replace('unit-', '')}`}
          </p>
        ...
        {/* 7. Bottom Navigation to Next Unit */}
        {hasNextUnit && onNextUnit && (
          <nav aria-label="Nadaljevanje" className="pt-8 border-t border-slate-200 dark:border-slate-800 flex justify-end">
            <button
              onClick={onNextUnit}
              className="flex items-center gap-2 px-5 py-2.5 rounded-lg bg-slate-900 hover:bg-slate-800 dark:bg-slate-100 dark:hover:bg-white text-white dark:text-slate-900 font-medium text-sm transition-all shadow-xs cursor-pointer"
            >
              <span>Nadaljuj na naslednjo enoto</span>
